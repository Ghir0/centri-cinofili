...
function StarRating({ rating, reviews }: { rating: number | null; reviews: number }) {
  if (rating === null) {
    return (
      <div className="flex items-center gap-1.5 text-xs text-[color:var(--ds-gray-500)]">
        <span aria-hidden className="text-[color:var(--ds-gray-400)]">☆☆☆☆☆</span>
  ...
  return (
    <div className="flex items-center gap-1.5 text-xs">
      <span aria-hidden className="text-amber-500 tracking-tight">
        {"★".repeat(full)}
        {half ? "½" : ""}
        <span className="text-[color:var(--ds-gray-200)]">{"★".repeat(5 - full - (half ? 1 : 0))}</span>
      </span>
      <span className="font-medium text-[color:var(--ds-gray-900)]">{rating.toFixed(1)}</span>
 ...
 * Layout denso, gerarchico, tutti i dati critici visibili senza click.
 */
export function CentroCard({ centro }: { centro: SearchResult }) {
  ...
  return (
    <article className="card p-5 flex flex-col gap-4 transition-shadow hover:shadow-[0_0_0_1px_rgba(0,0,0,0.08),0_4px_8px_-2px_rgba(0,0,0,0.06),0_8px_16px_-8px_rgba(0,0,0,0.05)]">
          ...
          <div className="flex items-start justify-between gap-3">
            <h3 className="text-h3 truncate">
              <Link
                ...
                className="hover:underline decoration-[color:var(--ds-gray-400)] underline-offset-4"
              >
                {displayName}
              </Link>
            </h3>
            <span
              className={`pill shrink-0 ${centro.claimed ? "pill-verified" : "pill-unclaimed"}`}
            >
              <span
                aria-hidden
                className={`inline-block h-1.5 w-1.5 rounded-full ${
                  centro.claimed ? "bg-[color:var(--ds-verified)]" : "bg-[color:var(--ds-gray-400)]"
                }`}
              />
              {centro.claimed ? "Verificato" : "Non verificato"}
            </span>
          </div>
      ...
      {/* Description preview */}
      {centro.ragione_sociale && centro.brand_name && centro.brand_name !== centro.ragione_sociale && (
        <p className="text-xs text-[color:var(--ds-gray-500)] font-mono truncate">
          {centro.ragione_sociale}
        </p>
      )}

      {/* Methodologies (most prominent taxonomy) */}
      {centro.metodologie.length > 0 && (
        <div>
          <div className="text-eyebrow mb-2">Metodo educativo</div>
          <div className="flex flex-wrap gap-1.5">
            {centro.metodologie.map((m) => (
              <span key={m.slug} className="pill">
                {m.nome}
              </span>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {centro.discipline.slice(0, 5).map((d) => (
              <span
                key={d.slug}
                className="text-[11px] px-2 py-0.5 rounded-md text-[color:var(--ds-gray-600)] bg-[color:var(--ds-gray-50)]"
              >
                {d.nome}
              </span>
            ))}
            {centro.discipline.length > 5 && (
              <span className="text-[11px] px-2 py-0.5 rounded-md text-[color:var(--ds-gray-500)]">
                +{centro.discipline.length - 5}
              </span>
            )}
          </div>
    ...
    return (
      <div className="card p-12 text-center">
        ...
        <p className="text-sm text-[color:var(--ds-gray-500)] max-w-md mx-auto">
          Prova a rimuovere alcuni filtri o a cercare un altro comune. Stiamo mappando attivamente
          tutta Italia: se manca una struttura, segnalacela.
        </p>
      </div>
    );
  }
